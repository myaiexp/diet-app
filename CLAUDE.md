...
- **Shared responses**: `notFound` / `badRequest` / `conflict` / `badGateway` / `serviceUnavailable` in `responses.ts`; write bodies via `readJsonBody` (`json-body.ts`) → Zod schemas under `schemas/`. Don't re-inline error shapes.
