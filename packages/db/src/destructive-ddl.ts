...
//
// diet-app has ONE database, `dietapp`, and helm copies the repo-root .env naming it
// into every worktree: `pnpm --filter @diet-app/db migrate` from a worktree writes
// PRODUCTION's schema. Additive DDL is safe by construction (the running code ignores a
// column it doesn't know about); the statements below are not — they remove or narrow a
// surface the deployed API may still be using, so they must land WITH the code that
// stopped using it. `migrate-guard.ts` turns these findings into a verdict.
//
// The scan is deliberately conservative in ONE direction only: a false positive costs a
// migration nothing but a slightly later application (the push-to-deploy hook runs
// drizzle-kit migrate from the main checkout right after landing), while a false
// negative is helm's 2026-08-22 incident — four columns dropped from a worktree, 25
// minutes of HTTP 500 on a live endpoint. Ported from helm's src/db/destructive-ddl.ts.
 ...
 *
 * This is what stops a migration's own prose from tripping the scan: 0114's comment
 * block explains at length which columns it drops and why, and a naive `rg DROP COLUMN`
 * over the file cannot tell that from the DDL two lines below it.
 *
