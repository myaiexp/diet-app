...
### user_disliked_ingredients (junction)

Disliked ingredients are normalized into a junction table rather than a `uuid[]` on `user_profile`: an array column can't carry a foreign key, so deleting an ingredient would orphan stale UUIDs and Drizzle can't define a relation over it.

| Column          | Type                                        | Notes        |
| --------------- | ------------------------------------------- | ------------ |
| `user_id`       | uuid (FK → user_profile, ON DELETE CASCADE) | Composite PK |
